...
function getData(p = null) {
    ...
    //console.log(p);
    loading.value = true
    const page = { "page": currentPage.value, "page_size": 10 }
...
const rules = {
    name: [{ required: true, message: '图库分类名称', trigger: 'blur' },]
}
...
const handleSubmit = () => {
    formRef.value.validate((valid) => {
        ...
        const data = { "id": editId.value ,"name":form.name,"order":form.order}
        const fun = editId.value ? updateImageClass(data) : createImageClass(form)
        fun.then(res => {
