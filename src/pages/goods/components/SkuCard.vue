<template>
    <el-form-item label="规格选项" v-loading="bodyLoading">
        <el-card shadow="never" class="w-full mb-3" v-for="(item, index) in sku_card_list" :key="item.id"
            ...
            <!-- card body -->
            <sku-card-item :sku-card-id="item.id"></sku-card-item>
        </el-card>
...
import { goodsId, sku_card_list, addSkuCardEvent, btnLoading, handleUpdate, handleDelete,sortCard,bodyLoading,handleChooseSetGoodsSkusCard } from "~/common/useSku"
